...
else{echo "<script>alert('select first');window.location='inbox.php'</script>";
}
}
 ...
 if($_POST['ch']>0)
  {
   foreach($_POST['ch'] as $v)
   {
    $sql=mysqli_query($con,"SELECT * FROM draft where user_id='$id' and draft_id='$v'");
    while($dd=mysqli_fetch_array($sql))
	{
	$rec=$dd['user_id'];
	$sen="you";
	...
	//delete form inbox
	mysqli_query($con,"DELETE FROM draft where user_id='$id' and draft_id='$v'");
    }
   }
   echo "<script>alert('message deleted');window.location='draft.php'</script>";
 }
 else{echo "<script>alert('select first');window.location='draft.php'</script>";}
}
